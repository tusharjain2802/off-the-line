class Aw {
  ...
  loadAssets(assetList) {
    ...
    assetList.forEach((assetName) => {
      ...
      if (assetName.endsWith(".png") || assetName.endsWith(".jpg")) {
        ...
        this.assets[assetName].data.onload = () =>
  ...
  isLoading() {
    return (
      ...
      Object.values(this.assets).every((asset) => asset.loaded) == false
    );
  }

  getAsset(assetName) {
    console.assert(
      this.assets[assetName] !== undefined,
      `No asset loaded named '${assetName}'`
    );
  ...
  gameLoop(curTime) {
    ...
    let deltaTime = Math.min(
      (curTime - (this.lastTime || curTime)) / 1000.0,
      0.2
    ); // Cap to 200ms (5fps)
  ...
  addEntity(entity) {
    ...
    entity._z =
      this.entities.length > 0
        ? this.entities[this.entities.length - 1].z + 1
        : 0;
  ...
  drawSprite(params) {
    ...
    let width =
      params.xScale !== undefined ? image.width * params.xScale : image.width;
    let height =
      params.yScale !== undefined ? image.height * params.yScale : image.height;
  ...
  drawText(params) {
    ...
    let textAlign =
      params.textAlign !== undefined ? params.textAlign.toLowerCase() : "left";
    let textBaseline =
      params.textBaseline !== undefined
        ? params.textBaseline.toLowerCase()
        : "bottom";
  ...
  playNote(note, octave, length, delay, type) {
    ...
    oscillator.start(
      this.audioCtx.currentTime + (delay !== undefined ? delay : 0)
    );
    oscillator.stop(
      this.audioCtx.currentTime +
        (delay !== undefined ? delay : 0) +
        (length !== undefined ? length : 0.2)
    );
  }
  ...
  initInput() {
    ...
    window.addEventListener("mousedown", (e) => {
      if (e.button === 0) {
        ...
        this.mouseRightButtonJustPressed = true;
      }
    });
  ...
  touch2Mouse(e) {
    ...
    // Create a MouseEvent using the modern constructor
    const mouseEvent = new MouseEvent(mouseEv, {
      bubbles: true,
      cancelable: true,
      view: window,
      detail: 0, // click count
  ...
  setKeyState(event, isOn) {
    ...
    if (this.keyToName[keyCode] !== undefined) {
      ...
      this.keysJustPressed[keyName] =
        this.keys[keyName] === false || this.keys[keyName] === undefined;
