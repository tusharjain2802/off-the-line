  ...
  createSegments() {
    for (let group = 0; group < this.linePoints.length; group++) {
      ...
      for (let i = 0; i < this.linePoints[group].length - 1; i++) {
        let xDist =
          this.linePoints[group][i + 1].x - this.linePoints[group][i].x;
        let yDist =
          this.linePoints[group][i + 1].y - this.linePoints[group][i].y;
      ...
      let xDist =
        this.linePoints[group][0].x -
        this.linePoints[group][this.linePoints[group].length - 1].x;
      let yDist =
        this.linePoints[group][0].y -
        this.linePoints[group][this.linePoints[group].length - 1].y;
      let segDist = Math.sqrt(xDist * xDist + yDist * yDist);
  ...
  getPosInfo(group, distance) {
    ...
    for (let i = 0; i < this.segLengths[group].length; i++) {
      ...
      if (distance >= curTotalDistance && distance < nextTotalDistance) {
        ...
        if (
          this.normals[group] !== undefined &&
          this.normals[group][i] !== undefined
        ) {
          return {
            x: xInterp,
            y: yInterp,
            nx: this.normals[group][i].x,
            ny: this.normals[group][i].y,
            xDir: xDir,
            yDir: yDir,
          };
        } else {
          let xDir =
            (this.linePoints[group][p2].x - this.linePoints[group][p1].x) /
            this.segLengths[group][i];
          let yDir =
            (this.linePoints[group][p2].y - this.linePoints[group][p1].y) /
            this.segLengths[group][i];
          return {
            x: xInterp,
            y: yInterp,
  ...
  getIntersectionInfo(x1, y1, x2, y2) {
    for (let group = 0; group < this.segLengths.length; group++) {
      ...
      for (let i = 0; i < this.segLengths[group].length; i++) {
        ...
        let lineIntersectInfo = getLineIntersectionInfo(
          this.linePoints[group][p1].x,
          this.linePoints[group][p1].y,
          this.linePoints[group][p2].x,
          this.linePoints[group][p2].y,
          x1,
          y1,
          x2,
          y2
        ...
        if (lineIntersectInfo.intersect) {
          lineIntersectInfo.distance =
            curTotalDistance +
            this.segLengths[group][i] * lineIntersectInfo.time;
