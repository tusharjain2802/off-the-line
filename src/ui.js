function drawUI(deltaTime) {
  ...
  if (difficultyMode == 2) {
    ...
    aw.ctx.fillRect(
      xStart,
      yStart,
      (level.timer / level.levelTime) * (screenWidth - 20),
      20
    );
  }
  ...
  aw.drawText({
    text: `LEVEL ${levelIdx + 1} - ${level ? level.name : ""}`,
    x: 10,
    y: 30,
    fontSize: 24,
    fontStyle: "bold",
  ...
  if (aw.state === gameOver) {
    if (levelIdx === 19) {
      ...
      aw.ctx.shadowColor = "#FFF";
      aw.drawText({
        text: `${getDifficultyModeName()} COMPLETE`,
        x: screenWidth * 0.5,
        y: 180,
        fontSize: 20,
        fontStyle: "bold",
